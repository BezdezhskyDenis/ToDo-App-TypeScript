...
const todoInvalidFeedback = document.querySelector(".todo-form .invalid-feedback");
renderTodoApp();
...
function renderTodoItem({ id, title, isComplete }) {
    return `
        ...
        >
          ...
          <label
            class="form-check-label mx-1 flex-fill ${isComplete ? "text-secondary text-decoration-line-through" : ""}"
            for="todo-id-${id}"
          >
            ${title}
          </label>
